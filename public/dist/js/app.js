...
if (formLogin) formLogin.onsubmit = async function(e) {
  ...
  try {
    const res = await axios.post('https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/login', {email, password});
...
document.querySelector('#btnFormSubmit').onclick = async function() {
  ...
  try {
    ...
    const res = await axios.post('https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/posts', form, {
      headers: {
...
async function submitLove(post_id) {
  try {
    const token = document.querySelector('meta[token]').getAttribute('token');
    const res = await axios.patch(`https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/posts/${post_id}/like`, {}, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
async function removeLove(post_id) {
  try {
    const token = document.querySelector('meta[token]').getAttribute('token');
    const res = await axios.delete(`https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/posts/${post_id}/like`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
async function fetchAllPosts() {
  try {
    const token = document.querySelector('meta[token]').getAttribute('token');
    const res = await axios.get('https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/posts', {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
...
if (btnLogout) btnLogout.onclick = async function() {
  try {
    const res = await axios.post('https://mukti-wibowo-laravel-code-test.herokuapp.com/api/v1/logout');
